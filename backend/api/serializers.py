...
class SubscribeSerializer(CustomUserSerializer):
    ...
    def get_recipes(self, obj):
        ...
        if recipes_limit:
            return RecipeShortSerializer(Recipe.objects.filter(author=obj)[:int(recipes_limit)], many=True).data
...
class CustomUserCreateSerializer(UserCreateSerializer):
    username = serializers.CharField(
        validators=[RegexValidator(r"^[a-zA-Z0-9]+([_.-]?[a-zA-Z0-9])*$", "Юзернейм может содержать только цифры, латинские буквы, знаки (не в начале): тире, точка и нижнее тире.")])
...
class BaseRecipeSerializer(serializers.ModelSerializer, IsFavoritedMixin, IsInShoppingCartMixin):
    ...
    ingredients = AmountIngredientSerializer(many=True, source='recipe_ingredient',
                                             )
...
class RecipeReadSerializer(BaseRecipeSerializer):
    ...
    ingredients = AmountIngredientSerializer(many=True, source='recipe_ingredient', read_only=True)
    tags = TagSerializer(many=True, read_only=True)
...
class RecipeCreateSerializer(BaseRecipeSerializer):
    ...
    def validate(self, validated_data):
        ...
        if len(name) > 150:
            raise serializers.ValidationError({'name': 'Название должно быть короче 150 символов!'})
        image = validated_data.get('image')
        if not image:
            raise serializers.ValidationError({'image': 'Поле изображения не может быть пустым!'})
        ...
        if len(ingredient_ids) != len(ingredients):
            raise serializers.ValidationError({'ingredients': 'Ингридиенты не должны повторяться!'})
...
class ShoppingCartCreateDeleteSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        serializer = RecipeShortSerializer(instance.recipe, context=self.context)
